...
###Cal-ViDa data
mortality = read.csv("respmortality1419.csv") #data from CalViDa-respmortality1423 for 2014-2019 bc we want to avoid COVID pandemic era
...
plot((CA_sf %>% mutate(clus = knn_clusterlabels$knn7))['clus'], main = "7 cluster example with population constraint")
```
...
mortality_ts_plot = ggplot(data = deaths_df, aes(x = date, y = deaths, group = Cluster, colour = Cluster)) + 
  geom_line() +
...
colnames(inla_full_data_wcovariates)[(end+1):(ncol(inla_full_data_wcovariates))] = c("lead","co","so2","no2","o3","pm10","pm25","aqi")
...
where the random effect $\textbf{F} | \tau \sim MVN(\textbf{0},\tau \boldsymbol{\Sigma})$
...
```{r}
pp_insample_plot = function(num_plots = num_clus, ref_data = inla_insample_data, pred_data,save_path = "C:/Users/jeffr/Desktop/Spatiotemporal + Causal Inference/Wildfire Paper 1 Code/Paper1-images/plos1-figs",prefix = ""){
  for (i in 1:num_plots){
    df = ref_data %>% filter(id == i) %>% select(response)
    preds = pred_data %>% filter(id == i) 
    df = cbind(df,preds)
...
pp_insample_plot_combined = function(num_plots = num_clus,
  ...
  # Arrange plots in a grid (2 rows × 4 cols for up to 8 clusters)
...
where $p(\boldsymbol{\phi}) \propto \exp(-\frac{1}{2} \sum_{c_1 \sim c_2} (\phi_{c_1} - \phi_{c_2})^2)$ and $\textbf{F} | \; \textbf{S}, \tau \sim MVN(\textbf{0},\tau \boldsymbol{\Sigma})$.
...
pp_insample_plot(pred_data = ref_model2_fit$fitted_values,prefix = "ref2-insample-")
...
kgr_model2 = function(dataset, rho_EPA_rbf = 1, rho_EPA_periodic = 1, sigma2_EPA = 1,link=1){
  ...
  #Heatmap of resulting K 
  inv_covGP_heatmap = matrix_heatmap2(inv_covGP,title = "",legend_title = "Precision",prefix = "kgr2-prec-7.2-")
...
kgr_model2_nb = function(dataset, rho_EPA_rbf = 1, rho_EPA_periodic = 1, sigma2_EPA = 1,link=1){
  ...
  #Heatmap of resulting K 
  inv_covGP_heatmap = matrix_heatmap2(inv_covGP,title = "",legend_title = "Precision",prefix = "kgr2-nb-prec-7.2-")
  
  ###Fit INLA model 
  kgr_formula2 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 +
    Intercept5 + Intercept6 + Intercept7 + f(id2,model = "generic0",Cmatrix = inv_covGP)
  
  # kgr_formula2 = response ~ -1 + offset(log(pop)) + months + f(id2,model = "generic0",Cmatrix = inv_covGP)
...
#Fit kgr_model2
kgr_model2_nb_fit = kgr_model2_nb(dataset = inla_insample_data,rho_EPA_rbf = 23.009,rho_EPA_periodic = 5253.017, sigma2_EPA = 0.928)

#Posterior predictive sampling from estimated intensities
# kgr_model2_nb_fit$fitted_values = nb_pp_sampling(kgr_model2_nb_fit$fitted_values,n=100000)

# #Extract DIC and WAIC 
# kgr_model2_DIC = kgr_model2_nb_fit$model_DIC
# kgr_model2_WAIC = kgr_model2_nb_fit$model_WAIC
# 
# #Get summaries of parameter estimates
# kgr_model2_fit$model_summary
# kgr_model2_fit$bri_hyperpar_summary
...
ref_model3 = function(dataset, cluster, rho_time_rbf = 1, rho_time_periodic = 1, sigma2_time = 1,link=1){
  ...
  inv_K_time_heatmap = matrix_heatmap2(inv_K_time,title = "",legend_title = "Precision",prefix = "ref3-prec-7.2-")
...
pp_insample_plot(num_plots = num_clus,ref_data = inla_insample_data,pred_data = ref_model3_fvs,prefix = "ref3-insample-")
pp_insample_plot_combined(num_plots = num_clus,ref_data = inla_insample_data,pred_data = ref_model3_fvs,prefix = "ref3-insample-")
```
...
ref_model4 = function(dataset, rho_time_rbf = 1, rho_time_periodic = 1, sigma2_time = 1, link=1){
  ...
  inv_covGP2_heatmap = matrix_heatmap2(inv_covGP2,title = "",legend_title = "Precision",prefix = "ref4-prec-7.2-")
...
#Fit ref_model4
ref_model4_fit = ref_model4(dataset = inla_insample_data_wcovariates,rho_time_rbf = 483.957,rho_time_periodic = 3.431,sigma2_time = 3.843)

#Posterior predictive sampling from estimated intensities
ref_model4_fit$fitted_values = poisson_pp_sampling(ref_model4_fit$fitted_values,n=100000)

#Extract DIC and WAIC
ref_model4_DIC = ref_model4_fit$model_DIC
ref_model4_WAIC = ref_model4_fit$model_WAIC
...
kgr_model1 = function(dataset, rho_time_rbf = 1, rho_time_periodic = 1, sigma2_time = 1, link=1){
  ...
  #Heatmap of resulting inv_covGP2 
  inv_covGP2_heatmap = matrix_heatmap2(inv_covGP2,title = "",legend_title = "Precision",prefix = "kgr1-prec-7.2-")
...
pp_insample_plot(pred_data = kgr_model1_fit$fitted_values,prefix = "kgr1-insample-")
...
kgr_model3 = function(dataset,rho_EPA_rbf = 1,rho_EPA_periodic = 1,rho_time_rbf = 1,rho_time_periodic = 1,sigma2 = 1,link=1){
 ...
 K_time = time_kernel(time_span = length(unique(dataset$time)),rho_rbf = rho_time_rbf, 
  ...
  #Heatmap of resulting K 
  inv_covGP3_heatmap = matrix_heatmap2(inv_covGP3,title = "",legend_title = "Precision",prefix = "kgr3-prec-7.2-")
...
kgr_model4 = function(dataset,rho_EPA_rbf = 1, rho_EPA_periodic = 1,
  ...
  inv_covGP4_heatmap = matrix_heatmap2(inv_covGP4,title = "",legend_title = "Precision",prefix = "kgr4-prec-7.2-")
...
pp_insample_plot(pred_data = kgr_model4_fit$fitted_values,prefix = "kgr4-insample-")
...
kgr_model5 = function(dataset, rho_AR_rbf = 1, rho_AR_periodic = 1, rho_DL_rbf = 1, rho_DL_periodic = 1,
  ...
  inv_covGP5_heatmap = matrix_heatmap2(inv_covGP5,title = "",legend_title = "Precision",prefix = "kgr5-prec-7.2-")
...
pp_insample_plot(pred_data = kgr_model5_fit$fitted_values,prefix = "kgr5-insample-")
...
plot((CA_sf %>% mutate(clus = clus7_response_min$groups))['clus'], main = "7 cluster example with population constraint")

plot((CA_sf %>% mutate(clus = clus7_response_minarea$groups))['clus'], main = "7 cluster example with population constraint")
```
...
pp_outsample_plot = function(num_plots = num_clus, ref_data = inla_full_data, pred_data){
  ...
  post_pred_plot = df %>% ggplot(aes(x=time,y=response)) + geom_point() + 
    geom_line(aes(y=mean),color = "red") + geom_ribbon(aes(ymin = `0.025quant`,ymax = `0.975quant`),alpha = 0.3) + geom_vline(xintercept = 54,linetype = "dashed",color = "blue",linewidth = 1.5) + ggtitle(title) + theme_classic()
  print(post_pred_plot)
  }
}
```
...
```{r}
pp_outsample_plot = function(num_plots = num_clus, ref_data = inla_full_data, pred_data,save_path = "C:/Users/jeffr/Desktop/Spatiotemporal + Causal Inference/Wildfire Paper 1 Code/Paper1-images/plos1-figs",prefix = ""){
  for (i in 1:num_plots){
    df = ref_data %>% filter(id == i) %>% select(response)
    preds = pred_data %>% filter(id == i) 
    df = cbind(df,preds)
    ...
    captured = (df$response[test_idx] >= df$y_pplower[test_idx] & df$response[test_idx] <= df$y_ppupper[test_idx])
    coverage = round(sum(captured)/length(captured)*100,2)
...
pp_outsample_plot(pred_data = ref_model1_outfit$fitted_values,prefix = "ref1-outsample-")
...
pp_outsample_plot(pred_data = ref_model2_outfit$fitted_values,prefix = "ref2-outsample-")
...
pp_outsample_plot(pred_data = kgr_model2_outfit$fitted_values,prefix = "kgr2-outsample-")
pp_outsample_plot_combined(pred_data = kgr_model2_outfit$fitted_values,prefix = "kgr2-outsample-")


kgr_model2_nb_outfit = kgr_model2_nb(data = inla_outsample_data,rho_EPA_rbf = 23.009,rho_EPA_periodic = 5253.017, sigma2_EPA = 0.928)
...
pp_outsample_plot(num_plots = num_clus,ref_data = inla_full_data,pred_data = ref_model3_outfvs,prefix = "ref3-outsample-")
...
```{r}
#Fit ref_model4
ref_model4_outfit = ref_model4(dataset = inla_outsample_data_wcovariates,rho_time_rbf = 483.957,rho_time_periodic = 3.431,sigma2_time = 3.843)

#Posterior predictive sampling from estimated intensities
ref_model4_outfit$fitted_values = poisson_pp_sampling(ref_model4_outfit$fitted_values,n=100000)
...
pp_outsample_plot(pred_data = ref_model4_outfit$fitted_values,prefix = "ref4-outsample-")
...
pp_outsample_plot(pred_data = kgr_model1_outfit$fitted_values,prefix = "kgr1-outsample-")
...
pp_outsample_plot(pred_data = kgr_model3_outfit$fitted_values,prefix = "kgr3-outsample-")
...
starting_data = inla_full_data[1:252,] #252 for 36 months 
...
while(max(starting_data$time) < 60){
  ...
  preds_ref_model2 = poisson_pp_sampling(preds_ref_model2)
  end2 = nrow(preds_ref_model2)

  pred_mean = preds_ref_model2$mean[(end2-6):end2]
  pred_lower = preds_ref_model2$y_pplower[(end2-6):end2]
  pred_upper = preds_ref_model2$y_ppupper[(end2-6):end2]
  
  starting_data$response[(end+1):(end+7)] = pred_mean
  starting_data$lower[(end+1):(end+7)] = pred_lower
  starting_data$upper[(end+1):(end+7)] = pred_upper
...
for (i in 1:num_clus){
  ...
  post_pred_plot = df %>% ggplot(aes(x=time,y=response)) + geom_point() + 
    ...
    geom_vline(xintercept = 36,linetype = "dashed",color = "blue",linewidth = 1.5) + ggtitle(title) + theme_classic()
  print(post_pred_plot)
}
```
...
starting_data2 = inla_full_data[1:252,] #252 for last 6 months 
starting_data2$months = as.numeric(starting_data2$months)
rownames(starting_data2) = NULL
starting_data2$lower = NA
starting_data2$upper = NA

MAE2 = c()
MAPE2 = c()
RMSPE2 = c()

while(max(starting_data2$time) < 60){
  
  ###Attach df for next month with NAs in response
  end = nrow(starting_data2)
  id = c(1:7)
  id2 = (starting_data2$id2[end]+1):(starting_data2$id2[end]+7)
  response = rep(NA,7)
  lower = rep(NA,7)
  upper = rep(NA,7)
  time = rep(starting_data2$time[end]+1,each=7)
  pop = cluster_pops
  Intercept1 = c(1,NA,NA,NA,NA,NA,NA)
  Intercept2 = c(NA,1,NA,NA,NA,NA,NA)
  Intercept3 = c(NA,NA,1,NA,NA,NA,NA)
  ...
  if (starting_data2$months[end] == 12){
    months = rep(1,7)
  } else{
    value = starting_data2$months[end]
    months = rep((value+1),7)
  }
  
  new_data = data.frame(id,id2,response,time,months,Intercept1,Intercept2,Intercept3,
                      Intercept4,Intercept5,Intercept6,Intercept7,pop,lower,upper)
  starting_data2 = rbind(starting_data2,new_data)
  starting_data2$months = factor(starting_data2$months)
  ...
  # #Proposed model 2
  # kgr_formula2 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 +
  #  Intercept5 + Intercept6 + Intercept7 + f(id,model = "generic0",Cmatrix = inv_covGP[c(starting_data2$id2),c(starting_data2$id2)])
  # 
  # kgr_model2 = inla(kgr_formula2, data = starting_data2, family = "poisson",
  #                   control.predictor = list(compute = TRUE, link = 1))
  # 
  ...
  #Proposed model 3
  # kgr_formula3 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 +
  #  Intercept5 + Intercept6 + Intercept7 + f(id,model = "generic0",Cmatrix = inv_covGP3[c(starting_data2$id2),c(starting_data2$id2)])
  # 
  # kgr_model3 = inla(kgr_formula3, data = starting_data2, family = "poisson",
  #                   control.predictor = list(compute = TRUE, link = 1))
  # 
  ...
  # #Proposed model 4
  # kgr_formula4 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 +
  #  Intercept5 + Intercept6 + Intercept7 + f(id2,model = "generic0",Cmatrix = inv_covGP4[c(starting_data2$id2),c(starting_data2$id2)])
  # 
  # kgr_model4 = inla(kgr_formula4, data = starting_data2, family = "poisson",
  #                   control.predictor = list(compute = TRUE, link = 1))
  # 
  ...
  # #Proposed model 5
  # kgr_formula5 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 + 
  #   Intercept5 + Intercept6 + Intercept7 + f(id,model = "generic0",Cmatrix = inv_covGP5[c(starting_data2$id2),c(starting_data2$id2)], hyper = prec_prior)
  # 
  # kgr_model5 = inla(kgr_formula5, data = starting_data2, family = "poisson",
  #                   control.inla = list(strategy = "laplace"),
  #                   control.predictor = list(compute = TRUE, link = 1)) 
  # 
  ...
  #NB model
  kgr_formula2 = response ~ -1 + months + Intercept1 + Intercept2 + Intercept3 + Intercept4 + 
    Intercept5 + Intercept6 + Intercept7 + f(id2,model = "generic0",Cmatrix = inv_covGP[c(starting_data2$id2),c(starting_data2$id2)])


  kgr_nb_model = inla(formula = kgr_formula2,family = "nbinomial",data = starting_data2,
                  control.compute = list(dic=TRUE,waic=TRUE,
                                         return.marginals.predictor=TRUE),
                  control.family = list(hyper = list(theta = list(prior = "pc.mgamma", param = 1))),
                  control.predictor = list(compute = TRUE, link = 1))

  preds_kgr_model = kgr_nb_model$summary.fitted.values
  ...
  # pp_ysamples = list()
  # n_density = data.frame(kgr_nb_model$marginals.hyperpar[[1]])
  # sample_n = inla.rmarginal(1000,inla.smarginal(n_density))
  # pp_nsamples = sample_n
  # 
  # ###Generate uncertainty bands from the posterior predictive distribution as opposed to posterior of the linear predictor
  # for (i in 1:nrow(preds_kgr_model)){
  #   ###Sample 1000 mu for each row
  #   row = data.frame(kgr_nb_model$marginals.fitted.values[[i]])
  # 
  #   sample_mu = inla.rmarginal(1000,inla.smarginal(row))
  #   lower = quantile(sample_mu,0.025)
  #   mean = mean(sample_mu)
  ...
  # 
  #   ###Obtain 95% credible interval
  #   preds_kgr_model$y_ppmean[i] = mean(sample_y, na.rm = TRUE)
  #   preds_kgr_model$y_pplower[i] = quantile(sample_y, 0.025, na.rm = TRUE)
  #   preds_kgr_model$y_ppupper[i] = quantile(sample_y, 0.975, na.rm = TRUE)
  # }
  
  pred_mean = preds_kgr_model$mean[(end2-6):end2]
  pred_lower = preds_kgr_model$y_pplower[(end2-6):end2]
  pred_upper = preds_kgr_model$y_ppupper[(end2-6):end2]

  starting_data2$response[(end+1):(end+7)] = pred_mean
  starting_data2$lower[(end+1):(end+7)] = pred_lower
  starting_data2$upper[(end+1):(end+7)] = pred_upper
  
  # starting_data2$response = replace(starting_data2$response,which(starting_data2$response < 0),0)
  starting_data2$months = as.numeric(starting_data2$months)
  ...
  MAE2 = c(MAE2,mean(abs(est_lambda - pred_mean)))
  MAPE2 = c(MAPE2,mean(abs((est_lambda - pred_mean)/est_lambda)))
  RMSPE2 = c(RMSPE2,sqrt(mean((est_lambda - pred_mean)^2)))
}
...
for (j in 36:60){
  for (c in 1:num_clus){
    actual = starting_data2 %>% filter(id == c)  %>% data.frame()
    actual_m = actual %>% filter(months == m,time < j) %>% select(response)
    est_lambda[c] = mean(actual_m$response)
  }
  
  pred_test = starting_data2 %>% filter(time == j) %>% select(response) %>% as.matrix() %>% c()
...
for (i in 1:num_clus){
  ...
  post_pred_plot = df %>% ggplot(aes(x=time,y=response)) + geom_point() + 
    geom_line(data = df %>% filter(time > 36), aes(y=mean),color = "red") + geom_ribbon(aes(ymin = lower,ymax = upper),alpha = 0.3) +
...
combined_data$MAE_ratio = combined_data$MAE[]
